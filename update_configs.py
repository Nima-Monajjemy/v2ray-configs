برای حل این چالش کلاسیک، باید تفاوت ماهیت شبکه‌ای سرورهای گیت‌هاب و اینترنت ایران را در نظر گرفت. سرورهای GitHub Actions در دیتاسنترهای ایزوله و بدون محدودیت (مانند Azure) قرار دارند؛ بنابراین هر کانفیگی که سرورِ مقصدِ آن خاموش نباشد، در آنجا پینگ می‌دهد و معتبر شناخته می‌شود. اما وقتی آن کانفیگ به اینترنت شما می‌رسد، توسط سیستم مانیتورینگ (DPI) اوپراتور تحلیل شده و اگر استانداردهای استتار ترافیک را رعایت نکرده باشد، مسدود (Drop) می‌شود.

راه‌حل بهینه، ایجاد یک لایه **پالایش اکتشافی (Heuristic Pre-Filtering)** در کد پایتون است. به این معنا که پیش از آنکه کانفیگ به نرم‌افزار Xray برای تست پینگ سپرده شود، ابتدا ساختار، پورت‌ها و پارامترهای آن (مانند `fp`، `security` و `type`) با الگوهای سخت‌گیرانه سیستم فیلترینگ تطبیق داده شود و کانفیگ‌های آسیب‌پذیر اساساً از چرخه تست خارج شوند. این کار علاوه بر افزایش چشمگیر خلوص کانفیگ‌های نهایی، سرعت اجرای کد در گیت‌هاب را نیز به شدت بهینه می‌کند.

### تغییرات اعمال شده در کد

۱. **افزودن تابع `is_iran_friendly_config**`: این تابع به عنوان یک گیت‌وی عمل کرده و بر اساس تحلیل قبلی، پورت‌های غیرمتعارف، Vmessهای خام (TCP بدون TLS)، و کانفیگ‌های Reality/TLS فاقد اثر انگشت (`fp`) را دور می‌ریزد.
۲. **اعمال فیلتر در تابع `extract_configs**`: کانفیگ‌ها در همان مرحله استخراج از تلگرام پالایش می‌شوند و فقط موارد ساختاریافته به لیست `configs` اضافه می‌شوند.

*فایل `yml` شما کاملاً استاندارد است و نیازی به تغییر ندارد. فقط کد پایتون زیر را جایگزین کد قبلی خود کنید:*
